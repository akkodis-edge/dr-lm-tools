...
void TestPSO::runTest()
{
    ...
    quint32 lowerBound = static_cast<quint32>(m_expectedFreq * 0.9);
    quint32 upperBound = static_cast<quint32>(m_expectedFreq * 1.1);
    
    qDebug("%s value: %uHz (Expected range: %uHz-%uHz)",
        getName().toStdString().c_str(), m_receivedFreq, lowerBound, upperBound);
}
